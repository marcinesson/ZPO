package pl.korepetycje.korepetycje.service;
...
public class TutoringBookingService {
    private final TutorSlotRepository resourceRepository;
    private final TutoringBookingRepository recordRepository;
    private final AppUserService appUserService;

    public List<TutorSlot> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<TutoringBooking> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<TutoringBooking> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public TutorSlot findBestResource(SubjectType category, int requiredCapacity,
    ...
    @Transactional
    public TutoringBooking createRecord(SubjectType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        TutorSlot resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        TutoringBooking record = new TutoringBooking();
    ...
    public BigDecimal calculateScore(TutorSlot resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(TutorSlot resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(TutorSlot resource, int taskUnits) {
    ...
    private void validateSearch(SubjectType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
