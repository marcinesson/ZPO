...
public interface DroneReservationRepository extends JpaRepository<DroneReservation, Long> {
    List<DroneReservation> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from DroneReservation record
