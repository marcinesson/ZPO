package pl.drony.drony.service;
...
public class DroneReservationService {
    private final InspectionDroneRepository resourceRepository;
    private final DroneReservationRepository recordRepository;
    private final AppUserService appUserService;

    public List<InspectionDrone> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<DroneReservation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<DroneReservation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public InspectionDrone findBestResource(DroneType category, int requiredCapacity,
            LocalDateTime startTime, LocalDateTime endTime) {
        validateSearch(category, requiredCapacity, startTime, endTime);

        return resourceRepository.findAll().stream()
                .filter(InspectionDrone::isAvailable)
                ...
                .sorted(Comparator.comparing(InspectionDrone::getCapacity)
                        .thenComparing(InspectionDrone::getPrice))
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No matching resource available."));
    }

    @Transactional
    public DroneReservation createRecord(DroneType category, int requiredCapacity,
            LocalDateTime startTime, LocalDateTime endTime) {
        InspectionDrone resource = findBestResource(category, requiredCapacity, startTime, endTime);
        DroneReservation record = new DroneReservation();
    ...
    public BigDecimal calculatePrice(InspectionDrone resource, int requiredCapacity,
            LocalDateTime startTime, LocalDateTime endTime) {
    ...
    private void validateSearch(DroneType category, int requiredCapacity,
            LocalDateTime startTime, LocalDateTime endTime) {
