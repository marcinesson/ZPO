...
public interface InstrumentRentalRepository extends JpaRepository<InstrumentRental, Long> {
    List<InstrumentRental> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from InstrumentRental record
