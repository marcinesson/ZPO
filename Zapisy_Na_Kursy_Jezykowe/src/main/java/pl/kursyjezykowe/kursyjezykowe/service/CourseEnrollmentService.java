package pl.kursyjezykowe.kursyjezykowe.service;
...
public class CourseEnrollmentService {
    private final LanguageGroupRepository resourceRepository;
    private final CourseEnrollmentRepository recordRepository;
    private final AppUserService appUserService;

    public List<LanguageGroup> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<CourseEnrollment> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<CourseEnrollment> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public LanguageGroup findBestResource(LanguageType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(LanguageGroup::isAvailable)
    ...
    @Transactional
    public CourseEnrollment createRecord(LanguageType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        LanguageGroup resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        CourseEnrollment record = new CourseEnrollment();
    ...
    public BigDecimal calculateScore(LanguageGroup resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(LanguageGroup resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(LanguageGroup resource, int taskUnits) {
    ...
    private void validateSearch(LanguageType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
