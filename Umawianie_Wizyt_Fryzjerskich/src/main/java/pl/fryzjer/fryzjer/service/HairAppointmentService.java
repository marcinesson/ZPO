package pl.fryzjer.fryzjer.service;
...
public class HairAppointmentService {
    private final HairdresserRepository resourceRepository;
    private final HairAppointmentRepository recordRepository;
    private final AppUserService appUserService;

    public List<Hairdresser> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<HairAppointment> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<HairAppointment> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public Hairdresser findBestResource(HairServiceType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(Hairdresser::isAvailable)
    ...
    @Transactional
    public HairAppointment createRecord(HairServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        Hairdresser resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        HairAppointment record = new HairAppointment();
    ...
    public BigDecimal calculateScore(Hairdresser resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(Hairdresser resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(Hairdresser resource, int taskUnits) {
    ...
    private void validateSearch(HairServiceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
