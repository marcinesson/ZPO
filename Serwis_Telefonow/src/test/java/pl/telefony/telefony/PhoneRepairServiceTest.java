...
import pl.telefony.telefony.service.AppUserService;
import pl.telefony.telefony.service.PhoneRepairService;

@ExtendWith(MockitoExtension.class)
class PhoneRepairServiceTest {
    @Mock
    private PhoneTechnicianRepository resourceRepository;

    @Mock
    private PhoneRepairRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private PhoneRepairService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, PhoneIssueType.SCREEN, 10, 5, 5, "100.00"),
                resource(2L, PhoneIssueType.SCREEN, 3, 3, 5, "70.00"),
                resource(3L, PhoneIssueType.BATTERY, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        PhoneTechnician result = service.findBestResource(PhoneIssueType.SCREEN,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, PhoneIssueType.SCREEN, 3, 3, 5, "70.00"),
                resource(2L, PhoneIssueType.SCREEN, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(PhoneIssueType.SCREEN,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        PhoneTechnician resource = resource(1L, PhoneIssueType.SCREEN,
                3, 3, 5, "100.00");
    ...
    private PhoneTechnician resource(Long id, PhoneIssueType category, int capacity,
            int quality, int unitsPerHour, String price) {
        PhoneTechnician resource = new PhoneTechnician();
        resource.setId(id);
