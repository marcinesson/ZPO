package pl.telefony.telefony.service;
...
import pl.telefony.telefony.repository.PhoneRepairRepository;
import pl.telefony.telefony.repository.PhoneTechnicianRepository;

@Service
@RequiredArgsConstructor
public class PhoneRepairService {
    private final PhoneTechnicianRepository resourceRepository;
    private final PhoneRepairRepository recordRepository;
    private final AppUserService appUserService;

    public List<PhoneTechnician> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<PhoneRepair> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<PhoneRepair> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public PhoneTechnician findBestResource(PhoneIssueType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(PhoneTechnician::isAvailable)
    ...
    @Transactional
    public PhoneRepair createRecord(PhoneIssueType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        PhoneTechnician resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        PhoneRepair record = new PhoneRepair();
    ...
    public BigDecimal calculateScore(PhoneTechnician resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(PhoneTechnician resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(PhoneTechnician resource, int taskUnits) {
    ...
    private void validateSearch(PhoneIssueType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
