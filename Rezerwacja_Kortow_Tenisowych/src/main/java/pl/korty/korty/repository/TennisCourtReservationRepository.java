...
public interface TennisCourtReservationRepository extends JpaRepository<TennisCourtReservation, Long> {
    List<TennisCourtReservation> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from TennisCourtReservation record
