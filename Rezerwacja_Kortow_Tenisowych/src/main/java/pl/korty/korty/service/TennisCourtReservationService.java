...
public class TennisCourtReservationService {
    private final TennisCourtRepository resourceRepository;
    private final TennisCourtReservationRepository recordRepository;
    private final AppUserService appUserService;

    public List<TennisCourt> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<TennisCourtReservation> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<TennisCourtReservation> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public TennisCourt findBestResource(CourtSurface category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(TennisCourt::isAvailable)
    ...
    @Transactional
    public TennisCourtReservation createRecord(CourtSurface category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        TennisCourt resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        TennisCourtReservation record = new TennisCourtReservation();
    ...
    public BigDecimal calculateScore(TennisCourt resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(TennisCourt resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(TennisCourt resource, int taskUnits) {
    ...
    private void validateSearch(CourtSurface category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
