package pl.przeprowadzki.przeprowadzki.service;
...
public class MovingOrderService {
    private final MovingTeamRepository resourceRepository;
    private final MovingOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<MovingTeam> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<MovingOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<MovingOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public MovingTeam findBestResource(MoveType category, int requiredCapacity,
    ...
    @Transactional
    public MovingOrder createRecord(MoveType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        MovingTeam resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        MovingOrder record = new MovingOrder();
    ...
    public BigDecimal calculateScore(MovingTeam resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(MovingTeam resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(MovingTeam resource, int taskUnits) {
    ...
    private void validateSearch(MoveType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
