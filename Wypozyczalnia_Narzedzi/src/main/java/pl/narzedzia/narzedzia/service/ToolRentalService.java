package pl.narzedzia.narzedzia.service;
...
public class ToolRentalService {
    private final ToolSetRepository resourceRepository;
    private final ToolRentalRepository recordRepository;
    private final AppUserService appUserService;

    public List<ToolSet> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<ToolRental> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<ToolRental> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public ToolSet findBestResource(ToolType category, int requiredCapacity,
    ...
    @Transactional
    public ToolRental createRecord(ToolType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        ToolSet resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        ToolRental record = new ToolRental();
    ...
    public BigDecimal calculateScore(ToolSet resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(ToolSet resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(ToolSet resource, int taskUnits) {
    ...
    private void validateSearch(ToolType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
