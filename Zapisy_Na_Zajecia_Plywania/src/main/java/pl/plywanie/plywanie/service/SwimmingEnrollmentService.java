package pl.plywanie.plywanie.service;
...
public class SwimmingEnrollmentService {
    private final SwimmingGroupRepository resourceRepository;
    private final SwimmingEnrollmentRepository recordRepository;
    private final AppUserService appUserService;

    public List<SwimmingGroup> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<SwimmingEnrollment> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<SwimmingEnrollment> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public SwimmingGroup findBestResource(SwimmingLevel category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(SwimmingGroup::isAvailable)
    ...
    @Transactional
    public SwimmingEnrollment createRecord(SwimmingLevel category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        SwimmingGroup resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        SwimmingEnrollment record = new SwimmingEnrollment();
    ...
    public BigDecimal calculateScore(SwimmingGroup resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(SwimmingGroup resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(SwimmingGroup resource, int taskUnits) {
    ...
    private void validateSearch(SwimmingLevel category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
