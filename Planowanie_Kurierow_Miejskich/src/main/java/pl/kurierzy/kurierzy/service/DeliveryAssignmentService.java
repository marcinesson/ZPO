package pl.kurierzy.kurierzy.service;
...
public class DeliveryAssignmentService {
    private final CourierVehicleRepository resourceRepository;
    private final DeliveryAssignmentRepository recordRepository;
    private final AppUserService appUserService;

    public List<CourierVehicle> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<DeliveryAssignment> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<DeliveryAssignment> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public CourierVehicle findBestResource(DeliveryZone category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(CourierVehicle::isAvailable)
    ...
    @Transactional
    public DeliveryAssignment createRecord(DeliveryZone category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        CourierVehicle resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        DeliveryAssignment record = new DeliveryAssignment();
    ...
    public BigDecimal calculateScore(CourierVehicle resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(CourierVehicle resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(CourierVehicle resource, int taskUnits) {
    ...
    private void validateSearch(DeliveryZone category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
