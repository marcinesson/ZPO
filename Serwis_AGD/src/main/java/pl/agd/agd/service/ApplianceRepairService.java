...
public class ApplianceRepairService {
    private final ServiceTechnicianRepository resourceRepository;
    private final ApplianceRepairRepository recordRepository;
    private final AppUserService appUserService;

    public List<ServiceTechnician> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<ApplianceRepair> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<ApplianceRepair> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public ServiceTechnician findBestResource(ApplianceType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(ServiceTechnician::isAvailable)
    ...
    @Transactional
    public ApplianceRepair createRecord(ApplianceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        ServiceTechnician resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        ApplianceRepair record = new ApplianceRepair();
    ...
    public BigDecimal calculateScore(ServiceTechnician resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(ServiceTechnician resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(ServiceTechnician resource, int taskUnits) {
    ...
    private void validateSearch(ApplianceType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
