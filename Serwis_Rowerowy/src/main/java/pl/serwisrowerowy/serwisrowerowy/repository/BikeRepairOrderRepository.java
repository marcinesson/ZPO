...
public interface BikeRepairOrderRepository extends JpaRepository<BikeRepairOrder, Long> {
    List<BikeRepairOrder> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from BikeRepairOrder record
