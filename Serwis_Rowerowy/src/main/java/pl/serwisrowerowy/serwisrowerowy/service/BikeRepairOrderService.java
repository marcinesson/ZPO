package pl.serwisrowerowy.serwisrowerowy.service;
...
public class BikeRepairOrderService {
    private final BikeMechanicRepository resourceRepository;
    private final BikeRepairOrderRepository recordRepository;
    private final AppUserService appUserService;

    public List<BikeMechanic> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<BikeRepairOrder> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<BikeRepairOrder> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public BikeMechanic findBestResource(RepairType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(BikeMechanic::isAvailable)
    ...
    @Transactional
    public BikeRepairOrder createRecord(RepairType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        BikeMechanic resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        BikeRepairOrder record = new BikeRepairOrder();
    ...
    public BigDecimal calculateScore(BikeMechanic resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(BikeMechanic resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(BikeMechanic resource, int taskUnits) {
    ...
    private void validateSearch(RepairType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
