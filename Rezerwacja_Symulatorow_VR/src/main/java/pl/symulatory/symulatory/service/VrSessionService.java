package pl.symulatory.symulatory.service;
...
public class VrSessionService {
    private final VrSimulatorRepository resourceRepository;
    private final VrSessionRepository recordRepository;
    private final AppUserService appUserService;

    public List<VrSimulator> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<VrSession> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<VrSession> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public VrSimulator findBestResource(VrScenario category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(VrSimulator::isAvailable)
    ...
    @Transactional
    public VrSession createRecord(VrScenario category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        VrSimulator resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        VrSession record = new VrSession();
    ...
    public BigDecimal calculateScore(VrSimulator resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(VrSimulator resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(VrSimulator resource, int taskUnits) {
    ...
    private void validateSearch(VrScenario category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
