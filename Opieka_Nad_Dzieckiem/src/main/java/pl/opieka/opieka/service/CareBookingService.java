...
public class CareBookingService {
    private final BabysitterRepository resourceRepository;
    private final CareBookingRepository recordRepository;
    private final AppUserService appUserService;

    public List<Babysitter> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<CareBooking> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<CareBooking> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public Babysitter findBestResource(CareType category, int requiredCapacity,
    ...
    @Transactional
    public CareBooking createRecord(CareType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        Babysitter resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        CareBooking record = new CareBooking();
    ...
    public BigDecimal calculateScore(Babysitter resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(Babysitter resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(Babysitter resource, int taskUnits) {
    ...
    private void validateSearch(CareType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
