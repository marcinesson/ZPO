...
public class MainRecordService {
    private final MainItemRepository itemRepository;
    private final MainRecordRepository recordRepository;
    private final AppUserService appUserService;

    public MainItem findBestAvailableItem(int requiredCapacity, LocalDateTime startTime, LocalDateTime endTime) {
        return itemRepository.findAll().stream()
            .filter(m -> m.isAvailable())
            .filter(m -> m.getCapacity() >= requiredCapacity)
            .filter(m -> recordRepository.findAll().stream()
                .noneMatch(record -> record.getItem().getId().equals(m.getId())
                    && record.getStartTime() != null
                    && record.getEndTime() != null
                    && record.getStartTime().isBefore(endTime)
                    && record.getEndTime().isAfter(startTime)))
            .sorted(Comparator.comparing(MainItem::getCapacity))
            .findFirst()
            .orElseThrow(() -> new RuntimeException("No available item found."));
    }

    public BigDecimal calculatePrice(MainItem item, LocalDateTime startTime, LocalDateTime endTime) {
        long hours = Math.max(1, Duration.between(startTime, endTime).toHours());
        return item.getPrice().multiply(BigDecimal.valueOf(hours));
    }

    public MainRecord createRecord(Long itemId, LocalDateTime startTime, LocalDateTime endTime) {
        validateDates(startTime, endTime);
        MainItem item = itemRepository.findById(itemId).orElseThrow(() -> new RuntimeException("Item not found."));
        Optional.of(item)
            .filter(m -> m.isAvailable())
    ...
    public List<MainRecord> getMyRecords() {
    ...
    public List<MainRecord> getAllRecords() {
