...
import pl.robotyka.robotyka.service.AppUserService;
import pl.robotyka.robotyka.service.RoboticsWorkshopService;

@ExtendWith(MockitoExtension.class)
class RoboticsWorkshopServiceTest {
    @Mock
    private RoboticsLabRepository resourceRepository;

    @Mock
    private RoboticsWorkshopRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private RoboticsWorkshopService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, RobotType.LEGO, 10, 5, 5, "100.00"),
                resource(2L, RobotType.LEGO, 3, 3, 5, "70.00"),
                resource(3L, RobotType.ARDUINO, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        RoboticsLab result = service.findBestResource(RobotType.LEGO,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, RobotType.LEGO, 3, 3, 5, "70.00"),
                resource(2L, RobotType.LEGO, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(RobotType.LEGO,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        RoboticsLab resource = resource(1L, RobotType.LEGO,
                3, 3, 5, "100.00");
    ...
    private RoboticsLab resource(Long id, RobotType category, int capacity,
            int quality, int unitsPerHour, String price) {
        RoboticsLab resource = new RoboticsLab();
        resource.setId(id);
