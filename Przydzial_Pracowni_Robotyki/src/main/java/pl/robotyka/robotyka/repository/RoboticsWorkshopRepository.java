...
public interface RoboticsWorkshopRepository extends JpaRepository<RoboticsWorkshop, Long> {
    List<RoboticsWorkshop> findByUserId(Long userId);

    @Query("""
            select count(record) > 0 from RoboticsWorkshop record
