package pl.robotyka.robotyka.service;
...
import pl.robotyka.robotyka.repository.RoboticsWorkshopRepository;
import pl.robotyka.robotyka.repository.RoboticsLabRepository;

@Service
@RequiredArgsConstructor
public class RoboticsWorkshopService {
    private final RoboticsLabRepository resourceRepository;
    private final RoboticsWorkshopRepository recordRepository;
    private final AppUserService appUserService;

    public List<RoboticsLab> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<RoboticsWorkshop> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<RoboticsWorkshop> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public RoboticsLab findBestResource(RobotType category, int requiredCapacity,
        ...
        return resourceRepository.findAll().stream()
                .filter(RoboticsLab::isAvailable)
    ...
    @Transactional
    public RoboticsWorkshop createRecord(RobotType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        RoboticsLab resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        RoboticsWorkshop record = new RoboticsWorkshop();
    ...
    public BigDecimal calculateScore(RoboticsLab resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(RoboticsLab resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(RoboticsLab resource, int taskUnits) {
    ...
    private void validateSearch(RobotType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
