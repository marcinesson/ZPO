package pl.badania.badania.service;
...
import pl.badania.badania.repository.LabTestBookingRepository;
import pl.badania.badania.repository.LabDeviceRepository;

@Service
@RequiredArgsConstructor
public class LabTestBookingService {
    private final LabDeviceRepository resourceRepository;
    private final LabTestBookingRepository recordRepository;
    private final AppUserService appUserService;

    public List<LabDevice> getAllResources() {
        return resourceRepository.findAll();
    }

    public List<LabTestBooking> getAllRecords() {
        return recordRepository.findAll();
    }

    public List<LabTestBooking> getMine() {
        return recordRepository.findByUserId(appUserService.getLoggedInUser().getId());
    }

    public LabDevice findBestResource(TestType category, int requiredCapacity,
    ...
    @Transactional
    public LabTestBooking createRecord(TestType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
        LabDevice resource = findBestResource(category, requiredCapacity, minQuality,
                taskUnits, priority, startTime, endTime);
        LabTestBooking record = new LabTestBooking();
    ...
    public BigDecimal calculateScore(LabDevice resource, int requiredCapacity,
    ...
    public BigDecimal calculatePrice(LabDevice resource, int taskUnits, int priority) {
    ...
    public int calculateRequiredHours(LabDevice resource, int taskUnits) {
    ...
    private void validateSearch(TestType category, int requiredCapacity,
            int minQuality, int taskUnits, int priority, LocalDateTime startTime, LocalDateTime endTime) {
