...
import pl.badania.badania.service.AppUserService;
import pl.badania.badania.service.LabTestBookingService;

@ExtendWith(MockitoExtension.class)
class LabTestBookingServiceTest {
    @Mock
    private LabDeviceRepository resourceRepository;

    @Mock
    private LabTestBookingRepository recordRepository;

    @Mock
    private AppUserService appUserService;

    @InjectMocks
    private LabTestBookingService service;
    ...
    void shouldChooseBestScoredResource() {
        ...
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, TestType.BLOOD, 10, 5, 5, "100.00"),
                resource(2L, TestType.BLOOD, 3, 3, 5, "70.00"),
                resource(3L, TestType.GENETIC, 3, 3, 5, "50.00")));
        when(recordRepository.existsOverlapping(anyLong(), eq(start), eq(end))).thenReturn(false);

        LabDevice result = service.findBestResource(TestType.BLOOD,
                3, 3, 10, 1, start, end);
    ...
    void shouldSkipOverlappingBetterResource() {
        ...
        LocalDateTime end = start.plusHours(3);
        when(resourceRepository.findAll()).thenReturn(List.of(
                resource(1L, TestType.BLOOD, 3, 3, 5, "70.00"),
                resource(2L, TestType.BLOOD, 5, 4, 5, "90.00")));
    ...
    void shouldRejectWhenTimeWindowIsTooShort() {
        ...
        assertThrows(RuntimeException.class,
                () -> service.findBestResource(TestType.BLOOD,
                        3, 3, 20, 1, start, end));
    }

    @Test
    void shouldCalculateStartedRequiredHoursAndPrice() {
        LabDevice resource = resource(1L, TestType.BLOOD,
                3, 3, 5, "100.00");
    ...
    private LabDevice resource(Long id, TestType category, int capacity,
            int quality, int unitsPerHour, String price) {
        LabDevice resource = new LabDevice();
        resource.setId(id);
